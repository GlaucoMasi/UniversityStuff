...
public class MainPane extends BorderPane {
	...
	public MainPane(Controller controller) {
		...
		leftBox.setPrefWidth(200);
			...
			localitaCombo.setOnAction(ev -> populateDateCombo());
			...
			buttonDettagliato.setOnAction(ev -> showPrevisione(TipoBollettino.DETTAGLIATO));
			
			buttonSintetico = new Button("Bollettino sintetico");
			buttonSintetico.setOnAction(ev -> showPrevisione(TipoBollettino.SINTETICO));
	...
	private void populateDateCombo() {
		dateCombo.setItems(FXCollections.observableArrayList(
				controller.getDatePerLocalita(localitaCombo.getValue())));
		dateCombo.getSelectionModel().selectFirst();
	}
	
	private void showPrevisione(TipoBollettino tipo) {
		outputArea.setText(controller.getBollettino(localitaCombo.getValue(), dateCombo.getValue(), tipo).toString());
	}
}
