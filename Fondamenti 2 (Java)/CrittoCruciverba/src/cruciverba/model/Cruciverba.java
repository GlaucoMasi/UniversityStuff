...
public class Cruciverba {
	...
	public Cruciverba(int numRighe, int numColonne) {
		if (numRighe<=0 || numColonne<=0) 
			throw new IllegalArgumentException("Cruciverba: numero di righe o colonne illegale: righe = " + numRighe + ", colonne = " + numColonne);
	...
	public void setParola(int indiceRiga, int indiceColonna, String s) {
		validaIndiceRiga(indiceRiga); validaIndiceColonna(indiceColonna);
		validaIndiceColonna(indiceColonna+s.length()-1);
		for(int i=0; i<s.length(); i++) {
			griglia[indiceRiga][indiceColonna+i] = s.charAt(i);
		}
	}
