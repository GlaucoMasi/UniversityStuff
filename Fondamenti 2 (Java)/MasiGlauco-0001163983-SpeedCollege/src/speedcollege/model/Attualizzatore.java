...
public class Attualizzatore {
	...
	public static BiFunction<Esame,LocalDate,Double> UN_VOTO_IN_MENO_AL_MESE =
			(esame, localdate) -> {
				...
				int mesi = Math.max(0, period.getYears()*12+period.getMonths());
				double votoAttualizzato = esame.getVotoIniziale().getValue().getAsInt()-mesi;
	...
	public double mediaPesata() {
		return carriera.getListaEsami().stream()
				...
				.collect(Collectors.summingDouble(this::votoPesato))
