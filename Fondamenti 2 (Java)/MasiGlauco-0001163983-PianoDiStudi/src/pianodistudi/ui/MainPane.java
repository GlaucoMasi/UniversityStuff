...
public class MainPane extends BorderPane {
	...
	public MainPane(Controller controller) {
		...
		leftBox.setPrefWidth(300);
			...
			afCombo = new ComboBox<String>(FXCollections.observableArrayList(controller.getListaAF()));
			afCombo.getSelectionModel().selectFirst();
			tipCombo = new ComboBox<Tipologia>(FXCollections.observableArrayList(Tipologia.values()));
	...
	private void myHandler(ActionEvent ev) {
		Tipologia t = tipCombo.getValue();
		
		try {
			if(ev.getSource().equals(inserisci)) controller.inserisci(controller.getAttivitaFormativaPerNome(afCombo.getValue()), t);
			else controller.rimuovi(controller.getAttivitaFormativaPerNome(afCombo.getValue()), t);
		} catch(IllegalArgumentException e) {
			Controller.alert("ERRORE!", "Errore nell'inserimento o nella rimozione dell'attività formativa", "Dettagli: " + e.getMessage());
		}
		
		statusBar.setText("CFU Totali: " + controller.getCfuTot());
		outputs.get(t).setText(controller.getAttivitaFormativePerTipologia(t)
				.stream()
				.map(AttivitaFormativa::toString)
				.collect(Collectors.joining(System.lineSeparator()))
				);
	}
	
	private void checkOrd(ActionEvent ev) {
		if(controller.rispettaOrdinamento()) outputVerifica.setText("Il piano soddisfa l'ordinamento");
		else outputVerifica.setText(controller.getLogVerificaOrdinamento());
	}
