...
public class BattleshipGrid extends GridPane {
	...
	static class CellButton extends Button {
		int row, col;
	...
	private final ComboBox<ShipItem> combo;
	...
	public void updateGridStatus() {
		for (int k = 0; k < tot; k++) {
			...
			Optional<Image> optImage = Optional.ofNullable(controller.getCurrentCellItem(row, col).getImage());
	...
	private void myHandle(ActionEvent e) {
		...
		source.setGraphic(new ImageView(value.getImage()));
